...
Khi hai biến cố là **độc lập** thì $P(B|A) = P(B)$:
...
viewof probSampling = {
  ...
  const popSvg = d3.create("svg").attr("viewBox",[0,0,gridW,gridH])
    .style("width","100%").style("border-radius","10px").style("background","#FAFBFD");
  ...
  popSvg.append("text")
    .attr("x", 14).attr("y", popZoneH + 24)
    .attr("fill", C.sub).attr("font-size","10px").attr("font-weight","700")
  ...
  const labelW=90, barH_=28, barGap=14, chartBarW=barChartW-labelW-60;
  const bY1=10, bY2=bY1+barH_+barGap;
  barSvg.append("text").attr("x",labelW-8).attr("y",bY1+barH_/2+1)
    .attr("text-anchor","end").attr("dominant-baseline","middle")
    .attr("fill",C.pos).attr("font-size","13px").attr("font-weight","700")
    .attr("font-family","Inter,sans-serif").text("Mắc bệnh");
  const barD = barSvg.append("rect").attr("x",labelW).attr("y",bY1)
    .attr("width",0).attr("height",barH_).attr("rx",6).attr("fill",C.pos).attr("opacity",0.85);
  const barDText = barSvg.append("text").attr("x",labelW+8).attr("y",bY1+barH_/2+1)
    .attr("dominant-baseline","middle").attr("fill",C.txt)
    .attr("font-size","14px").attr("font-weight","800").attr("font-family",mono).text("0");
  barSvg.append("text").attr("x",labelW-8).attr("y",bY2+barH_/2+1)
    .attr("text-anchor","end").attr("dominant-baseline","middle")
    .attr("fill",C.neg).attr("font-size","13px").attr("font-weight","700")
    .attr("font-family","Inter,sans-serif").text("Khỏe mạnh");
  const barHRect = barSvg.append("rect").attr("x",labelW).attr("y",bY2)
    .attr("width",0).attr("height",barH_).attr("rx",6).attr("fill",C.neg).attr("opacity",0.85);
  const barHText = barSvg.append("text").attr("x",labelW+8).attr("y",bY2+barH_/2+1)
    .attr("dominant-baseline","middle").attr("fill",C.txt)
    .attr("font-size","14px").attr("font-weight","800").attr("font-family",mono).text("0");
