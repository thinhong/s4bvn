...
```{ojs}
//| echo: false
viewof ace_sim_final = Inputs.form({
  draw: Inputs.button("Deal 10 Cards"),
  auto: Inputs.toggle({label: "Auto Run", value: false}),
  reset: Inputs.button("Reset Data")
})

{
  ...
  const CHART_THEO = "rgba(255, 255, 255, 0.1)";
  const CHART_ACTUAL = "#64ffda"; 
  const CHART_ACTIVE = "#1de9b6"; 
  ...
  const S = this.stateFinal;

  if (ace_sim_final.reset) {
     S.counts = [0,0,0,0,0];
     S.totalDeals = 0;
     S.lastHand = [];
     S.animation = 0;
  ...
  function runDeal() {
    ...
    const startX = (width - (5 * (CARD_W + 10))) / 2 + 5;
    S.lastHand = hand.map((c, i) => {
       ...
       return {
         ...c,
         x: 30, y: 30, 
         targetX: startX + col * (CARD_W + 10),
         targetY: 80 + row * (CARD_H + 15),
         t: 0
       };
    });
    S.animation = 1;
  ...
  if (ace_sim_final.draw) runDeal();

  // 4. LOOP
  function update() {
    const form = viewof ace_sim_final;
    ...
    if (S.animation === 1) {
       ...
       S.cooldown++;
       // FIX 1: Slow down auto-run (30 -> 60 frames = ~1 sec delay)
       if (S.cooldown > 60) runDeal(); 
    }
  }
  ...
  function drawCard(c, x, y) {
    ...
    if (c.isAce) {
        ctx.fillStyle = ACE_BG;
        ctx.beginPath(); ctx.roundRect(x, y, CARD_W, CARD_H, 4); ctx.fill();
        ctx.lineWidth = 2; ctx.strokeStyle = ACE_COLOR; ctx.stroke();
        ctx.shadowBlur = 0; ctx.shadowOffsetY = 0;
        
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.fillStyle = ACE_COLOR;
        ctx.font = "bold 18px serif";
        ctx.fillText("A", x + CARD_W/2, y + CARD_H/2 - 5);
        ctx.font = "16px sans-serif";
        ctx.fillText(c.suit, x + CARD_W/2, y + CARD_H/2 + 12);
    } else {
        ctx.fillStyle = CARD_BG_STD;
        ctx.beginPath(); ctx.roundRect(x, y, CARD_W, CARD_H, 4); ctx.fill();
        ctx.lineWidth = 1; ctx.strokeStyle = "#bdc3c7"; ctx.stroke();
        ctx.shadowBlur = 0; ctx.shadowOffsetY = 0;

        const isRed = (c.suit === "♥" || c.suit === "♦");
        ctx.fillStyle = isRed ? CARD_RED : CARD_BLACK;

        ctx.textAlign = "left";
        ctx.textBaseline = "top";
        ctx.font = "bold 14px sans-serif";
        ctx.fillText(c.rank, x + 4, y + 5);
        ctx.font = "16px sans-serif";
  ...
  function draw() {
    ...
    ctx.fillText(`Hands Dealt: ${S.totalDeals}`, 80, 30);
    
    if (isAutoRunning) {
        ctx.fillStyle = CHART_ACTUAL;
        ctx.fillText("● Auto", 80, 50);
    } else if (S.lastHand.length > 0) {
       const aces = S.lastHand.filter(c=>c.isAce).length;
       ctx.fillStyle = ACE_COLOR;
       ctx.fillText(`Result: ${aces} Aces`, 80, 50);
    }

    for(let i=0; i<3; i++) {
      ctx.fillStyle = CARD_BG_STD;
      ctx.fillRect(30-i, 30-i, CARD_W, CARD_H);
      ctx.strokeStyle = "#bdc3c7";
      ctx.strokeRect(30-i, 30-i, CARD_W, CARD_H);
    }
    ...
    // --- CHART ---
    const chartY = 380; // Moved slightly down
    // FIX 2: Reduced Height (120 -> 90) to prevent overlap
    const chartH = 90; 
    const chartX = 60;
    const barW = 50;
    const gap = 30;
    ...
    ctx.fillText("Aces in Hand (k)", width/2 + 15, chartY + 18);
    ...
    for (let k = 0; k <= 4; k++) {
       ...
       ctx.fillStyle = (S.animation === 0 && S.totalDeals > 0 && k === activeAces) 
                       ? CHART_ACTIVE : CHART_ACTUAL;
       
       ctx.fillRect(x, chartY - barH, barW, barH);
       
       ctx.fillStyle = TEXT_COLOR;
       ctx.font = "bold 12px sans-serif";
       ctx.fillText(k, x + barW/2, chartY + 12);
       
       if (pct > 0) {
          ctx.fillStyle = (S.animation === 0 && k === activeAces) ? CHART_ACTIVE : TEXT_COLOR;
          ctx.font = "10px sans-serif";
          // Label logic stays same, fits better with shorter bars
