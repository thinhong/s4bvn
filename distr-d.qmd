...
```{ojs}
//| echo: false
viewof coin_sim_simple = Inputs.form({
...

{
  ...
  // OPTIMIZATION: High-DPI support + Alpha false for speed
  ...
  // 2. STATE MANAGEMENT
  if (!this.stateCoinSimple) {
    this.stateCoinSimple = {
       history: [], 
       counts: {},  
       totalFlips: 0,
       currentResult: [], 
       animation: 0, 
       cooldown: 0,
       lastFlipVal: coin_sim_simple.flip,
       lastResetVal: coin_sim_simple.reset
    };
    for(let i=0; i<=N; i++) this.stateCoinSimple.counts[i] = 0;
  }
  const S = this.stateCoinSimple;
  ...
  // Handle Reset Button
  if (coin_sim_simple.reset > S.lastResetVal) {
     doReset();
     S.lastResetVal = coin_sim_simple.reset;
  }

  // Main Logic: Run a single experiment (flip 10 coins)
  function runFlip() {
     let heads = 0;
     const res = [];
     for(let i=0; i<N; i++) {
  ...
  // Handle Flip Button
  if (coin_sim_simple.flip > S.lastFlipVal) {
     runFlip();
     S.lastFlipVal = coin_sim_simple.flip;
  }

  // 4. RENDER LOOP
  function update() {
    const form = viewof coin_sim_simple;
  ...
  function draw() {
    ...
    if (S.currentResult.length > 0) {
       ...
       ctx.textAlign = "right";
       ctx.fillStyle = TEXT_COLOR;
       ctx.fillText(`Số mặt Ngửa: ${hCount} / ${N}`, width - 20, 30);
    }
    ...
    // Scale Y-axis based on P=0.5 constant
    // Max prob for N=10, P=0.5 is at k=5 (~0.246)
    // We hardcode scale max to keep chart stable
