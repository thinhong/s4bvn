...
viewof venn = (() => {
  ...
  // ══════════════════════════════════════════════════════
  const SL = {};
  SL.pA = createSlider("P(A)", 0.01, 0.95, 0.01, 0.05, "#5b9bd5", "blue");
  SL.pB = createSlider("P(B)", 0.01, 0.95, 0.01, 0.05, "#e8915a", "amber");
  ...
  function update() {
    ...
    if (isMutEx) {
      ...
      const dir = pAB > pApB ? "Positive" : "Negative";
  ...
  // EVENTS
  // ══════════════════════════════════════════════════════
  function onInput() { SL.pA.sync(); SL.pB.sync(); update(); }
  SL.pA.input.addEventListener("input", onInput);
  SL.pB.input.addEventListener("input", onInput);
  update();

  invalidation.then(() => {
    SL.pA.input.removeEventListener("input", onInput);
    SL.pB.input.removeEventListener("input", onInput);
  });
