...
viewof coin_sim_manual = (() => {
  ...
  container.innerHTML = `
    ...
    <div class="coin-controls-row">
      <button id="btn-flip" class="coin-btn coin-btn-primary">Tung 10 đồng xu</button>
  ...
  function draw() {
    ...
    if (S.currentResult.length > 0) {
       ...
       ctx.textAlign = "right"; ctx.fillStyle = TEXT_COLOR;
...
viewof control_sharp = (() => {
  ...
  container.innerHTML = `
    <style>
      ...
      .ctrl-btn:hover { background-color: #f3f4f6; }
      
      /* Primary Button (Blue) for Start */
      .ctrl-btn-primary {
        background-color: #3b82f6; 
        color: white; 
        border-color: #2563eb;
      }
      .ctrl-btn-primary:hover { background-color: #2563eb; color: white; }
    </style>

    <div class="ctrl-row">
      <button id="btn-start" class="ctrl-btn ctrl-btn-primary">Bắt đầu</button>
      <button id="btn-stop" class="ctrl-btn">Dừng</button>
    </div>
  `;
...

{
  ...
  // 1. CENTERED CONTAINER FOR CANVAS
  // We wrap the canvas in a flex div to center it on the page
  ...
  // Add canvas to the centered wrapper
  ...
  // We need to re-bind these because the cell re-runs when inputs change
  // Note: ideally controls and canvas would be in one cell to avoid re-binding issues,
  // but following your requested structure, we hook into the DOM elements.
  if (startBtn) {
      startBtn.onclick = () => {
        balls = [];
  ...
  // Return the wrapper (div), not just the canvas
